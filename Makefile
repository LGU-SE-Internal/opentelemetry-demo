...
lint-json:
	...
	@find . -type f -name '*.json' \
		...
		-not -path '*/Pods/*' \
		-not -path '*/.expo/*' \
		-print0 | xargs -0 -I {} sh -c 'jq . {} > /dev/null || (echo "ERROR: Invalid JSON in {}" && exit 1)'
...
.PHONY: lint
lint: misspell markdownlint yamllint checklicense lint-json
...
start:
	...
	@echo "Go to http://localhost:8080/fe
