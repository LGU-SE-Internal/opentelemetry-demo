...
install-ruff:
    # Using a venv is recommended
	ruff --version >/dev/null 2>&1 || pip install -U ruff~=$(RUFF_VERSION)
...
lint-python: install-ruff
	ruff check src/load-generator/ --select E,F,W --ignore E501
...
.PHONY: check
check: misspell markdownlint checklicense checklinks lint-python
...
start:
	...
	@echo "Go to http://localhost:8080/telemetry/ for the Weaver generated telemetry documenta
