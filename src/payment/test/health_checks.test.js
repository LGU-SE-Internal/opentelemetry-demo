const chargeModule = require('../charge');
const { isHealthy } = chargeModule;
...
describe('Payment Service Health Check Acceptance Criteria', () => {
  ...
  beforeEach(() => {
    ...
    connectivityCheckSpy = jest.spyOn(chargeModule, 'checkPaymentProcessorConnectivity');
    isHealthySpy = jest.spyOn(chargeModule, 'isHealthy');
  ...
  afterEach(() => {
    ...
    connectivityCheckSpy.mockRestore();
  ...
  test('ac1_isHealthy_returns_true_when_connectivity_succeeds', async () => {
    connectivityCheckSpy.mockResolvedValue(true);
  ...
  test('ac2_isHealthy_returns_false_when_connectivity_fails', async () => {
    connectivityCheckSpy.mockResolvedValue(false);
    const result = await isHealthy();
  ...
  test('ac3_isHealthy_returns_false_on_timeout_within_threshold', async () => {
    ...
    // Mock checkPaymentProcessorConnectivity to take 200ms to respond
    connectivityCheckSpy.mockImplementation(() => new Promise(resolve => {
  ...
  test('ac4_ready_endpoint_returns_503_when_unhealthy', async () => {
    isHealthySpy.mockResolvedValue(false);
    const response = await request(app).get('/ready');
  ...
  test('ac8_mock_connectivity_fails_approximately_10_percent_of_time', async () => {
    ...
    for (let i = 0; i < totalRuns; i++) {
      const result = await chargeModule.checkPaymentProcessorConnectivity();
      if (!result) {
        failureCount++;
      }
    }
