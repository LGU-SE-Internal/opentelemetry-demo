...
type TestLogger struct {
	loggedWarns []map[string]interface{}
}
...
func (l *TestLogger) Warn(ctx context.Context, msg string, fields ...zap.Field) {
	...
	for _, f := range fields {
		logEntry[f.Key] = f.Interface
	}
	l.loggedWarns = append(l.loggedWarns, logEntry)
}
...
func TestAC1_OversizedMessageFailsValidation(t *testing.T) {
	...
	logger := &TestLogger{}
	validator, err := NewMessageValidator(config, logger)
